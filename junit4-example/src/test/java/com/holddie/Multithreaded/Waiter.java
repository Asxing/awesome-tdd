 ...
 */
package com.holddie.Multithreaded;
...
public class Waiter {
    ...
    public void assertNotNull(Object object) {
        if (object == null) fail("expected not null");
    }
    ...
    public void assertNull(Object object) {
        if (object != null) fail(format("null", object));
    }
    ...
    /**
     * Waits until the {@code delay} has elapsed, {@link #resume()} is called, or the test is
     * failed.
     *
    ...
    /**
     * Waits until the {@code delay} has elapsed, {@link #resume()} is called {@code
     * expectedResumes} times, or the test is failed.
     *
     ...
     * @throws AssertionError if any assertion fails while waiting
     */
    public void await(long delay, int expectedResumes)
     ...
     * @throws AssertionError if any assertion fails while waiting
     */
    public void await(long delay, TimeUnit timeUnit, int expectedResumes)
            throws TimeoutException, InterruptedException {
        try {
            if (failure == null) {
                synchronized (this) {
                    int remaining = remainingResumes.addAndGet(expectedResumes);
                    if (remaining > 0) circuit.open();
                }

                if (delay == 0) circuit.await();
                else if (!circuit.await(delay, timeUnit)) {
                    final int actualResumes = expectedResumes - remainingResumes.get();
                    throw new TimeoutException(
                            String.format(TIMEOUT_MESSAGE, expectedResumes, actualResumes));
                }
            }
        } finally {
    ...
    public synchronized void resume() {
        if (remainingResumes.decrementAndGet() <= 0) circuit.close();
    }
    ...
    /**
     * Fails the current test with the given {@code reason}, sets the number of expected resumes to
     * 0, and throws the {@code reason} as an {@code AssertionError} in the main test thread and in
     * the current thread.
     *
    ...
    public void fail(Throwable reason) {
        ...
        if (reason instanceof AssertionError) ae = (AssertionError) reason;
        else {
