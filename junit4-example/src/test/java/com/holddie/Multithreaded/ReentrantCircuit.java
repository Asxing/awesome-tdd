...
public class ReentrantCircuit {
    ...
    private static final class Sync extends AbstractQueuedSynchronizer {
        ...
        protected int tryAcquireShared(int acquires) {
            ...
            if (queuedThread != null && queuedThread != Thread.currentThread()) return -1;

            // If await test
            if (acquires == 0) return isClosed() ? 1 : -1;
    ...
    /**
     * Waits for the {@code waitDuration} until the circuit has been closed, aborting if
     * interrupted, returning true if the circuit is closed else false.
     */
