...
public class CompletableFutureAssertionsExamples extends AbstractAssertionsExamples {
    ...
    public void completableFuture_assertions_examples() {
        ...
        assertThat(future).succeedsWithin(100, TimeUnit.MILLISECONDS, as(STRING)).startsWith("oo");
        // same assertions with Duration
        assertThat(future).succeedsWithin(Duration.ofMillis(100)).isEqualTo("ook!");
        assertThat(future).succeedsWithin(Duration.ofMillis(100), as(STRING)).startsWith("oo");
        ...
        try {
            assertThat(completedFuture).isCompletedExceptionally().isNotCancelled();
        } catch (AssertionError e) {
    ...
    public void completionStage_assertions_examples() {
        ...
        assertThat(actual).isDone().isNotCancelled().isNotCompletedExceptionally();
    }
}
