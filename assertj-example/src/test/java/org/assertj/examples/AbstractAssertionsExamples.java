...
public abstract class AbstractAssertionsExamples {
    ...
    /** log error message if one wants to see it "live". */
    protected static void logAssertionErrorMessage(
            ThrowingCallable assertions, String assertionContext) {
        AssertionError assertionError = catchThrowableOfType(assertions, AssertionError.class);
        logger.info(
                ERROR_MESSAGE_EXAMPLE_FOR_ASSERTION, assertionContext, assertionError.getMessage());
    }
    ...
    protected final List<Ring> ringsOfPower =
            newArrayList(oneRing, vilya, nenya, narya, dwarfRing, manRing);
    ...
    protected final Movie theFellowshipOfTheRing =
            new Movie("the fellowship of the Ring", parse("2001-12-19"), "178 min");
    protected final Movie theTwoTowers =
            new Movie("the two Towers", parse("2002-12-18"), "179 min");
    protected final Movie theReturnOfTheKing =
            new Movie("the Return of the King", parse("2003-12-17"), "201 min");
    protected final Movie theSilmarillion =
            new Movie("the Silmarillion", parse("2030-01-01"), "unknown");
    protected final List<Movie> trilogy =
            newArrayList(theFellowshipOfTheRing, theTwoTowers, theReturnOfTheKing);

    // Various comparators
    protected Comparator<TolkienCharacter> ageComparator = new AgeComparator();
    protected Comparator<TolkienCharacter> raceNameComparator =
            new TolkienCharacterRaceNameComparator();
    ...
    protected Comparator<Character> caseInsensitiveComparator =
            new CaseInsensitiveCharacterComparator();
    ...
    public static void setUpOnce() {
        ...
        doubleDoubleStats =
                new Condition<BasketBallPlayer>("double double stats") {
                    @Override
                    public boolean matches(BasketBallPlayer player) {
                        if (player.getPointsPerGame() >= 10 && player.getAssistsPerGame() >= 10)
                            return true;
                        if (player.getPointsPerGame() >= 10 && player.getReboundsPerGame() >= 10)
                            return true;
                        if (player.getAssistsPerGame() >= 10 && player.getReboundsPerGame() >= 10)
                            return true;
                        return false;
                    }
                };
