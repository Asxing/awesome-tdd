...
/**
 * Shows some example of a custom assertion class: {@link TolkienCharacterAssert} that allows us to
 * make assertions specific to {@link TolkienCharacter}.
 *
...
public class CustomAssertExamples extends AbstractAssertionsExamples {
    ...
    public void successful_custom_assertion_example() {
        ...
        // To have a unique entry point for your custom assertions and the core ones, create a class
        // inheriting from
        // Assertions and add an assertThat(TolkienCharacter) that build a TolkienCharacterAssert,
        // then import statically
        // MyProjectAssertions.assertThat to have a unique entry point to all assertions : yours and
        // the core ones.
        ...
        assertThat(frodo).hasName("Frodo").hasAge(33).hasRace(Race.HOBBIT).isNotEqualTo(merry);
        // - isEqualTo is accessible since MyProjectAssertions inherits from Assertions which
        // provides Integer assertions.
    ...
    public void failed_custom_assertion_example() {
        sam.setName("Sammy");
        try {
            // custom assertion : assertThat is resolved from MyProjectAssertions.assertThat static
            // import
            assertThat(sam).hasName("Sam");
        } catch (AssertionError e) {
            // As we are defining custom assertion, we can set meaningful assertion error message,
            // like this one :
        ...
        try {
            ...
            assertThat(e)
                    .hasMessage(
                            "[check name] Expected character's name to be <Sam> but was <Sammy>");
        }
    }
    ...
    public void inherited_assertion_example() {
        ...
        EmployeeOfTheMonthAssert.assertThat(employeeOfTheMonth)
                .forMonth(1)
                .isEqualTo(employeeOfTheMonth);
        EmployeeOfTheMonthAssert.assertThat(employeeOfTheMonth)
                .isEqualTo(employeeOfTheMonth)
                .as("")
                .hasName("John Doe")
    ...
    public void multiple_soft_custom_assertions_combined_with_standard_ones_example() {
        ...
        logAssertionErrorMessage(
                () -> softly.assertAll(),
                "Two custom SoftAssertions combined with the standard one example");
    }
