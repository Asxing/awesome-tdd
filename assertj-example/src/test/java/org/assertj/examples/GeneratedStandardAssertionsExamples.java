...
public class GeneratedStandardAssertionsExamples extends AbstractAssertionsExamples {
    ...
    public void generated_standard_assertions_example() throws NameException {
        ...
        assertThat(cavs)
                .as("cavs players")
                .hasPlayers(rose, james, wade)
                .doesNotHavePlayers(antetokounmpo);
        ...
        org.assertj.examples.data.movie.Team lotr =
                new org.assertj.examples.data.movie.Team(
                        newArrayList("vigo mortensen", "elijah wood"));
        ...
        assertThat(test).hasOnlyMovies(theFellowshipOfTheRing).doesNotHaveMovies(theTwoTowers);

        Movie movie = new Movie("boom", new Date(), "1h");
        movie.setCreator("foo");
        assertThat(movie).hasCreator("foo").hasTitle("boom");
        try {
            movie = null;
