...
public class AssertionsWithCustomComparatorExamples extends AbstractAssertionsExamples {
    ...
    public void string_assertions_with_custom_comparison_examples() {
        ...
        // All assertions called after usingComparator(caseInsensitiveStringComparator) are based on
    ...
    public void collection_assertions_with_custom_comparator_examples() {
        ...
        try {
            assertThat(newArrayList(gandalf))
    ...
    public void date_assertions_with_custom_comparison_examples() {
        ...
        // build date away from today by one day (if we are at the end of the month we subtract one
        // day, otherwise we add one)
        Date oneDayFromTodayInSameMonth =
                monthOf(tomorrow()) == monthOf(new Date()) ? tomorrow() : yesterday();
